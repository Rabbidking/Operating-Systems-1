...
#define FAT_ENTRIES (500*1024*1024/4096*4)
static unsigned fat[FAT_ENTRIES];
...
int file_read(int fd, void* buf, unsigned count)
{
	...
	unsigned clusterNum = fp->firstCluster;
	...
	if(fp->in_use != 0)
	{
		...
		unsigned offsetInBuffer = fp->pos % 4096;	//bytes into the current cluster we currently are
		unsigned clustersToSkip = fp->pos / 4096;	//how many clusters into the file we are
		
		while(clustersToSkip > 0)
		{
			clusterNum = fat[clusterNum];
		}
		readCluster(clusterNum, someBuffer);
